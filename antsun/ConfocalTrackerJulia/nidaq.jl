...
function init_nidaq()
    ...
    # global task_ai = analog_input("$NIDAQ_DEV_NAME/ai0, $NIDAQ_DEV_NAME/ai1, $NIDAQ_DEV_NAME/_ao1_vs_aognd", ## was _ao0_vs_aognd
    #     terminal_config=NIDAQ.Differential, range=[0,10])
        # If you want to record thermistor readings too:
    global task_ai = analog_input("$NIDAQ_DEV_NAME/ai0, $NIDAQ_DEV_NAME/ai1, $NIDAQ_DEV_NAME/_ao1_vs_aognd, $NIDAQ_DEV_NAME/ai4", ## was _ao0_vs_aognd
        terminal_config=NIDAQ.Differential, range=[0,10])
    # digital: confocal camera AUX 1 OUT, behavior camera
    global task_di = digital_input("$NIDAQ_DEV_NAME/port0/line0:1")

    # Set up laser analog output
    global task_laser = analog_output("$NIDAQ_DEV_NAME/ao0") # Maybe add: , range=[0,10] ?
    ...
    # AO - Laser doesn't need a clock setup, since we want to be able to change the laser as we get positional info
end
