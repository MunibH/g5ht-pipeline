Base.@kwdef mutable struct Param
    ...
    # food patch polygon, point spacing for lines
    line_space_px = 10 # pixels
end
...
Base.@kwdef mutable struct SessionData
    ...
    # Laser / Temperature
#     list_laser_info = Array{Float64,3}[]
#     laser_enabled::Bool = false
#     laser_temp::Float64 = NaN
#     laser_pow::Float64 = NaN

#     # Stage cooling
#     list_cooling_info = Array{Float64,3}[]
#     stage_temp::Float64 = NaN
#     cooling_flow_rate::Float64 = NaN

    # Stage Zeroing
    cooling_zero_right_x::Float64 = NaN # Redo
    cooling_zero_right_y::Float64 = NaN
    cooling_zero_left_x::Float64 = NaN # Redo
    cooling_zero_left_y::Float64 = NaN
    laser_zero_x::Float64 = NaN 
    laser_zero_y::Float64 = NaN 

    cooling_zero_calc_x::Float64 = NaN
    cooling_zero_calc_y::Float64 = NaN
    
    # Track distance to square food patch defined by four points
    # if you're facing the microscope, the points are defined as:
    #    
    #   TL----TR
    #   |      |
    #   |      |
    ...
    polygonCalculated = false
    
    Line_TL_TR = [NaN NaN] # (x,y)
    Line_TR_BR = [NaN NaN]
    Line_BR_BL = [NaN NaN]
    Line_BL_TL = [NaN NaN]
...
#     session.list_laser_info = Array{Float64,3}[]
end
...
function save_h5(path_h5; metadata::Union{Nothing,Dict{String,Any}}=nothing, save_images::Bool=true)
...
#     laser_record = vcat(session.list_laser_info...)
#     cooling_record = vcat(session.list_cooling_info...)
...
#         write(h5f, "cooling_zero_right_x", session.cooling_zero_right_x)
#         write(h5f, "cooling_zero_right_y", session.cooling_zero_right_y)
#         write(h5f, "cooling_zero_left_x", session.cooling_zero_left_x)
#         write(h5f, "cooling_zero_left_y", session.cooling_zero_left_y)
#         write(h5f, "cooling_laser_x", session.laser_zero_x)
#         write(h5f, "cooling_laser_y", session.laser_zero_y)

#         laser_record_f64 = [Float64.(x) for x in laser_record] # Might not be necessary
#         write(h5f, "laser_record", laser_record_f64)
        
#         cooling_record_f64 = [Float64.(x) for x in cooling_record] # Might not be necessary
#         write(h5f, "cooling_record", cooling_record_f64)
        
        dist2patch_mm = [Float64.(x) for x in session.dist2patch_mm]
        write(h5f, "dist2patch_mm", dist2patch_mm)
