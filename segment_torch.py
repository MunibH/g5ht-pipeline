...
def main():
    ...
    DEVICE = f"cuda:{sys.argv[2]}" if len(sys.argv) > 2 else "cpu"
    
    # make sure cuda device can be found
    if "cuda" in DEVICE and not torch.cuda.is_available():
        print(f"CUDA device specified ({DEVICE}) but not available. Falling back to CPU.")
        DEVICE = "cpu"
        
    # make sure there's at least 8 GB on DEVICE
    if "cuda" in DEVICE:
        free_mem = torch.cuda.get_device_properties(DEVICE).total_memory - torch.cuda.memory_allocated(DEVICE)
        if free_mem < 8 * 1024**3:
            print(f"Warning: CUDA device {DEVICE} has less than 8 GB free memory. Consider using CPU or a different GPU.")
    ...
    for i in range(0, Z, BATCH_SIZE):
        ...
        batch_pred = predict_batch(model, batch_tensor, DEVICE)
