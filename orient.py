...
def main():
    ...
    if len(sys.argv) >= 4:
        # nose position provided as command-line args
        ...
        # fall back to orient_nose.csv in the dataset directory
        csv_path = os.path.join(spline_path, 'orient_nose.csv')
        if not os.path.exists(csv_path):
            raise FileNotFoundError(
                f'No nose position args provided and orient_nose.csv not found at {csv_path}')
        df = pd.read_csv(csv_path)
        nose_row = df[df['frame'] == 0].iloc[0]
        last_pt = [np.clip(int(nose_row['nose_y']), 0, 512),
                   np.clip(int(nose_row['nose_x']), 0, 512)]
        constraint_rows = df[df['frame'] != 0].sort_values('frame')
        if len(constraint_rows) > 0:
            constrain_frames = [int(r['frame']) for _, r in constraint_rows.iterrows()]
            constrain_noses = [[np.clip(int(r['nose_y']), 0, 512),
                                np.clip(int(r['nose_x']), 0, 512)]
                               for _, r in constraint_rows.iterrows()]
        else:
            constrain_frames = None
            constrain_noses = None
