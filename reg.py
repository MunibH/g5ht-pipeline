...
def register(fixed_stack, fixed_mask_stack, moving_stack, moving_mask_stack):
	...
	moving_gfp, moving_rfp = moving_stack[:, 0].copy().astype(np.float32), moving_stack[:, 1].copy().astype(np.float32)
	...
	registered_rfp, transform_parameters = itk.elastix_registration_method(fixed_image, moving_image, parameter_object,
																		   fixed_mask=fixed_mask, moving_mask=moving_mask)
	moving_gfp_image = itk.image_from_array(moving_gfp)
	registered_gfp = itk.transformix_filter(moving_gfp_image, transform_parameters)
