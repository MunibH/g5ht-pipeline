...
def register(fixed, moving, parameter_object, threads=8):
    ...
    fixed_img = itk.image_from_array(np.ascontiguousarray(fixed))
    moving_img = itk.image_from_array(np.ascontiguousarray(moving))
    ...
    return itk.elastix_registration_method(fixed_img, moving_img, #fixed_mask=fixed_mask, moving_mask=moving_mask,
...
def shear_correct(stack, parameter_object):
    ...
    for i in range(max_rfp_z - 1, -1, -1):
        ...
        gfp_reg = itk.transformix_filter(itk.image_from_array(np.ascontiguousarray(output[i, 0])), params)
        output[i] = np.asarray(gfp_reg), np.asarray(rfp_reg)
