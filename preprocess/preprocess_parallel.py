...
noise_path = '/home/munib/CODE/g5ht-pipeline/noise_042925.tif'
noise_tif = tifffile.imread(noise_path)
noise_stack = np.stack([noise_tif] * 41, axis=0).astype(np.float32)

def get_stack(input_nd2, index, stack_shape=(41, 2, 512, 512), trim=2):
...
def process_one(index, input_nd2, out_dir):
    ...
    if not os.path.exists(tif_path):
        ...
        stack = get_stack(input_nd2, index)
...
def main():
    ...
    n_workers = int(sys.argv[4]) if len(sys.argv) > 4 else cpu_count()
    ...
    with Pool(processes=n_workers) as pool:
        for _ in tqdm.tqdm(pool.imap_unordered(
                partial(process_one, input_nd2=input_nd2, out_dir=out_dir),
                indices),
                total=len(indices)):
            pass
