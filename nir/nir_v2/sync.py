...
def sync_timing(di_confocal: np.ndarray,
    ...
    return confocal_to_nir, nir_to_confocal, timing_stack, timing_nir, timing_piezo


def sync_timing_from_h5(path_h5: str,
                        n_rec: int = 1) -> Tuple[list, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    ai_laser = daqmx_ai[:,0] # filter_ai_laser(daqmx_ai[:, 0], daqmx_di[:, 0], n_rec)
    ai_piezo = daqmx_ai[:, 1].astype(np.float32)
    di_nir   = daqmx_di[:, 1].astype(np.float32)
    di_confocal = daqmx_di[:, 0].astype(np.float32)

    return sync_timing(di_confocal, ai_laser, di_nir, ai_piezo, img_id, q_iter_save, n_img_nir)
